 ...
 * 包含 ProseMirror（MIT，https://prosemirror.net）及其依赖。平台版本 0.2.1。 */
var LidaijiEditor = (() => {
  ...
  __export(editor_exports, {
    ...
    createStudioEditor: () => createStudioEditor,
  ...
  var IMAGE_BLOCK = /^!\[([^\]]*)\]\(([^)\s]+)\)\s*$/;
  var IMAGE_INLINE = /^!\[([^\]]*)\]\(([^)\s]+)\)/;
  var SHORTCODE_OPEN = /^\{\{< (align) (?:"?)(left|center|right)("?) >\}\}$|^\{\{< (poetry|endnote) >\}\}$/;
  ...
  function isWordChar(char) {
    return char !== void 0 && WORD_CHAR.test(char);
  }
  function parseInline(text) {
    ...
    while (index < text.length) {
      ...
      if (char === "\\") {
        buffer += (_a = text[index + 1]) != null ? _a : "\\";
        index += 2;
        continue;
      }
      ...
      if (triple === "***" || triple === "___") {
        ...
        const none2 = !marks.has("strong") && !marks.has("em");
        if (both) {
          marks.delete("strong");
          marks.delete("em");
        } else if (none2) {
      ...
      if (char === "`") {
        ...
        flush();
        const content = text.slice(index + run, close2);
        if (content) {
      ...
      if (link) {
        ...
        inline.push({
          type: "link",
          href: link[2],
          title: (_c = (_b = link[3]) != null ? _b : link[4]) != null ? _c : "",
  ...
  function serializeInline(nodes) {
    ...
    for (const node of nodes || []) {
      if (node.type === "hard_break") {
        ...
        output += `![${(_a = node.alt) != null ? _a : ""}](${node.src})`;
      } else if (node.type === "code") {
        const delimiter = codeDelimiter((_b = node.text) != null ? _b : "");
        output += `${delimiter}${(_c = node.text) != null ? _c : ""}${delimiter}`;
      } else if (node.type === "link") {
        const inner = serializeInline(node.content);
        const title = node.title ? ` "${node.title}"` : "";
        output += `[${inner}](${node.href}${title})`;
      } else {
        const text = escapeInline((_d = node.text) != null ? _d : "");
...
{{< /align >}}`;
      ...
      case "table": {
        const rows = (block.content || []).map(
          (row) => (row.content || []).map((cell) => escapeCell(serializeInline(cell.content)))
        );
      ...
      case "image":
        return `![${(_a = block.alt) != null ? _a : ""}](${block.src})`;
      case "code_block": {
        let longest = 0;
        let run = 0;
...
${fence}`;
      ...
      case "endnote_block": {
        ...
        const inner = (block.content || []).map((child) => serializeBlock(child, { stripPid: true })).filter((part) => part !== "").join("\n\n");
        return inner ? `{{< ${name} >}}
...
{{< /${name} >}}`;
  ...
  function inlineToPm(node) {
    ...
    if (node.type === "text") return schema.text((_a = node.text) != null ? _a : "", marksFromJson(node.marks));
    if (node.type === "hard_break") return schema.nodes.hard_break.create();
    if (node.type === "image") return schema.nodes.image.create({ src: (_b = node.src) != null ? _b : "", alt: (_c = node.alt) != null ? _c : "" });
    if (node.type === "code") return schema.text((_d = node.text) != null ? _d : "", [schema.marks.code.create()]);
    if (node.type === "link") {
      const linkMark = schema.marks.link.create({ href: (_e = node.href) != null ? _e : "", title: (_f = node.title) != null ? _f : "" });
      const parts = [];
      for (const child of node.content || []) {
        if (child.type === "text") {
          parts.push(schema.text((_g = child.text) != null ? _g : "", [...marksFromJson(child.marks), linkMark]));
        } else if (child.type === "code") {
          parts.push(schema.text((_h = child.text) != null ? _h : "", [schema.marks.code.create(), linkMark]));
  ...
  function inlinesFromPm(node) {
    ...
    for (const child of node.content.content) {
      if (child.type.name === "text") {
        ...
        if (linkMark) {
          if (!linkBuffer || linkBuffer.href !== linkMark.attrs.href || linkBuffer.title !== linkMark.attrs.title) {
            flushLink();
          ...
          if (rest.some((mark) => mark.type.name === "code")) {
            linkBuffer.content.push({ type: "code", text: (_a = child.text) != null ? _a : "" });
          } else {
            const names = rest.map((mark) => mark.type.name).sort();
            const entry = { type: "text", text: (_b = child.text) != null ? _b : "" };
        ...
        if (child.marks.some((mark) => mark.type.name === "code")) {
          result.push({ type: "code", text: (_c = child.text) != null ? _c : "" });
        } else {
          const names = child.marks.map((mark) => mark.type.name).sort();
          const entry = { type: "text", text: (_d = child.text) != null ? _d : "" };
          if (names.length) entry.marks = names;
          result.push(entry);
        }
        ...
        flushLink();
        result.push({ type: "image", src: (_e = child.attrs.src) != null ? _e : "", alt: (_f = child.attrs.alt) != null ? _f : "" });
      }
    }
    flushLink();
    return result;
    ...
    switch (block.type) {
      ...
      case "bullet_list":
        return schema.nodes.bullet_list.create(
          null,
          (block.content || []).map(
            (item) => schema.nodes.list_item.create(null, schema.nodes.paragraph.create(null, (item.content || []).flatMap(inlineToPm)))
          )
        );
      case "ordered_list":
      ...
      case "table": {
        const rows = (block.content || []).map(
          (row) => schema.nodes.table_row.create(
            null,
  ...
  function flattenInlines(parentNode2) {
    var _a, _b;
    const result = [];
    for (const child of parentNode2.content.content) {
      if (child.type.name === "paragraph") {
        result.push(...inlinesFromPm(child));
      } else if (child.type.name === "image") {
        result.push({ type: "image", src: (_a = child.attrs.src) != null ? _a : "", alt: (_b = child.attrs.alt) != null ? _b : "" });
      }
    }
    return result;
  }
  function jsonToPm(jsonDoc) {
    let content = ((jsonDoc == null ? void 0 : jsonDoc.content) || []).map(blockToPm);
  ...
  function blockToJson(node) {
    ...
    switch (node.type.name) {
      ...
      case "image":
        return { type: "image", src: (_a = node.attrs.src) != null ? _a : "", alt: (_b = node.attrs.alt) != null ? _b : "" };
      case "code_block":
        return { type: "code_block", info: (_c = node.attrs.info) != null ? _c : "", text: node.textContent };
      default:
        return { type: "paragraph", pid: null, content: inlinesFromPm(node) };
    }
  }
  function pmToJson(doc3) {
    return { type: "doc", content: doc3.content.content.map(blockToJson) };
  }
  function textOffsetAt(doc3, pos) {
    const target = Math.max(0, Math.min(pos, doc3.content.size));
    let offset = 0;
    doc3.nodesBetween(0, target, (node, nodeStart) => {
      if (node.isText) {
  ...
  function positionForTextOffset(doc3, target) {
    ...
    while (low < high) {
      ...
      if (textOffsetAt(doc3, mid) < target) low = mid + 1;
      else high = mid;
    }
  ...
  function containerExitEnter(state, dispatch) {
    ...
    const remaining = [];
    for (let i2 = 0; i2 < container.childCount - 1; i2 += 1) remaining.push(container.child(i2));
